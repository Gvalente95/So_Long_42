...
/*   Updated: 2024/11/08 03:31:23 by gvalente         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "header.h"

void	handle_movement(t_ent *e, int base_speed_x, int base_speed_y)
{
	int	adjusted_speed;
...
{
	if (e->pos.y < WIN_H - e->size.y - 1 && !e->jump_timer)
		e->action = FALL;
	if (e->jump_timer > 0)
	{
		e->jump_timer--;
	...
	if (e->pos.y > WIN_H - e->size.y + 1)
	{
		if (e->action == FALL)
			e->action = IDLE;
		e->movement.y = 0;
		e->pos.y = WIN_H - e->size.y;
	}
	if (e->action == FALL)
	{
		e->movement.y+=10;
		handle_movement(e, speed_x, speed_y); 		
	}
	else
	{
		if (frm_mv.x != 0)
	...
	if ((frm_mv.x == 0 && frm_mv.y == 0) && e->movement.x == 0 && e->movement.y == 0)
	{
...
int update_mobs(t_mlx_data *mlx_data)
{
	...
	while (++i < MOB_AMOUNT)
	{
		...
		if (i == 0)
			printf("MOB[0]: %s JMP_TIMER: %d\n", action_names[mlx_data->MOBS[0].action], mlx_data->MOBS[0].jump_timer);
	}
	return (1);
}
...
int	update_player(t_mlx_data *mlx_data)
{
	...
	{
		mlx_data->player.jump_timer = 8;
		new_mv.y = -10;
	}
	else if (mlx_data->key_state[DOWN_KEY] || mlx_data->key_state[S_KEY])
		new_mv.y = 1;
	if (mlx_data->key_state[SPACE_KEY] && mlx_data->player.jump_timer == 0)
	{
		mlx_data->player.jump_timer = 8;
		new_mv.y = -4;
	}
