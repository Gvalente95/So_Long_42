...
/*   Updated: 2024/11/08 03:55:48 by gvalente         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mlx_data(t_mlx_data *mlx_data)
{
	...
	mlx_data->background_img = mlx_new_image(mlx_data->mlx, WIN_W, WIN_H);
	if (!mlx_data->background_img)
		return (printf("Error creating background image\n"), 0);
	mlx_data->addr = mlx_get_data_addr(mlx_data->background_img, &mlx_data->bits_per_pixel, &mlx_data->line_length, &mlx_data->endian);
...
int	init_mobs(t_mlx_data *mlx_data, char *frames_path, int ammount)
{
	...
	while (++i < ammount)
	{
		...
		set_Vec3(&ent->pos, rand_range(0, WIN_W), rand_range(0, WIN_H), 1);
...
int	init_player(t_mlx_data *mlx_data, char *frames_path)
{
	...
	player->flip_X = 0;
	set_Vec3(&player->pos, WIN_W / 2, WIN_H / 2, 0);
...
int	init_env(t_mlx_data *mlx_data)
{
	...
	i = -1;
	while (++i < 4)
		imgs[i] = add_img(frames[rand_range(0, 4)], &size[i].x, &size[i].y, mlx_data->mlx);
	i = -1;
	while (++i < ENV_AMOUNT)
	{
		e = &mlx_data->ENVIRONMENT[i];
		r = rand_range(0, 4);
		set_Vec3(&e->pos, rand_range(0, WIN_H), rand_range(0, WIN_W), 0);
		set_Vec3(&e->movement,0,0,0);
		e->size = size[r];
		e->type = env;
		e->cur_frame = imgs[0];
		ft_printf("ENV FRAME: %s", frames[r]);
	}
	free(frames);
	return (1);
}
